...
int increment_password(char *password, const char *charset, int charset_len, int password_len) {
    ...
    for(int i = password_len-1; i >= 0;i--){
        ...
        if(index == -1){
            printf("Erro! Caracter não esta no charset\n");
            return 0;
        }
...
void save_result(int worker_id, const char *password) {
    ...
    if(fd >= 0){
        ...
        printf("Erro ao criar o arquivo %s", RESULT_FILE);
    }
}
    ...
    while (1) {
        ...
        if(passwords_checked%PROGRESS_INTERVAL == 0) {
            if(check_result_exists()){
                printf("Encontrado o resultado! Parando worker %d ...\n",worker_id);
                break;
            }
        }
        ...
        //printf("senha a ser TESTADA:%s --- ", current_password);
        //printf("%s\n", computed_hash);
        ...
        if(strcmp(current_password, end_password) > 0){
            printf("Fim do intervalo do worker %d\n", worker_id);
            break;
        }
